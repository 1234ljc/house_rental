<template>
  <div class="change-password">
    ...
    <el-card class="password-card">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" style="max-width: 450px;">
    ...
    <el-card class="tips-card">
      <template #header><span>安全提示</span></template>
      <ul class="tips-list">
        <li>密码长度为6-20个字符</li>
...
const validateConfirmPassword = (rule: any, value: string, callback: any) => {
  ...
  else if (value !== form.value.newPassword) callback(new Error('两次输入的密码不一致'))
  else callback()
}

const rules: FormRules = {
  oldPassword: [{ required: true, message: '请输入当前密码', trigger: 'blur' }],
  newPassword: [{ required: true, message: '请输入新密码', trigger: 'blur' }, { min: 6, max: 20, message: '密码长度为6-20个字符', trigger: 'blur' }],
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      await updatePasswordApi({ oldPassword: form.value.oldPassword, newPassword: form.value.newPassword })
